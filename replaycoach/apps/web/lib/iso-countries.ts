/**
 * ISO 3166-1 alpha-2 country list for the Geo Access Control admin UI's
 * searchable multi-select. Static and bundled — no dependency, no API call.
 * Codes match what ip-api.com (and most IP geolocation providers) return in
 * their `countryCode` field, so admin selections compare directly against
 * detected locations with no translation layer.
 */
...
export function countryNameForCode(code: string | null): string {
  if (!code) return 'Unknown';
  return ISO_COUNTRIES.find((c) => c.code === code.toUpperCase())?.name ?? code;
}
