/**
 * ISO 3166-1 *numeric* -> alpha-2 crosswalk for the geo-analytics
 * choropleth (`CountryChoropleth.tsx`). The bundled world-atlas topology
 * (`public/world-countries-110m.json`, from `world-atlas`'s
 * `countries-110m.json`) IDs every feature by numeric code as a
 * zero-padded string (e.g. `"840"` for the US) — it carries no alpha-2
 * property — while this app's own data (`geo_access_logs.country_code`,
 * `GeoCountryStat.countryCode`) is alpha-2. Without this table the map
 * couldn't be colored by our data at all.
 *
 * Covers the 174 of the topology's 177 features that have a standard ISO
 * 3166-1 alpha-2 code. The other 3 (Somaliland, Northern Cyprus, Kosovo)
 * are disputed territories world-atlas includes as separate shapes but
 * that aren't in the ISO 3166-1 list — they render as "no data" on the
 * map (never crash), and this app's own ISO_COUNTRIES list
 * (iso-countries.ts) doesn't carry codes for them either, so this isn't a
 * new gap. Spot-checked: US -> 840, GB -> 826.
 */
...
export function alpha2ForNumericCode(numericId: string): string | null {
  return ISO_NUMERIC_TO_ALPHA2[numericId] ?? null;
}
